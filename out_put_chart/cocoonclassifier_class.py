...
class MODEL():
    ...
    def read_train_data(self): 
        ...
        trainSet, valSet = torch.utils.data.random_split(data, [num_train_data, num_val_data])
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            for param in ["train", "val"]:
                ...
                epoch_loss = torch.true_divide(running_loss,len(self.dataset[param]))
                epoch_correct = torch.true_divide(100*running_correct,len(self.dataset[param]))
            ...
            if epoch % 5 == 0:
              path = "models/{}/{}/bs_{}".format(self.train_mode, self.modelname, self.batch_size)
              if not os.path.isdir(path):
                os.mkdir(path)
              torch.save(self.model, "{}/eph_{}_bs_{}_1355&929_{}_finetune".format(path, self.batch_size, epoch, self.batch_size, self.modelname))
    ...
    def read_list(self, list_path):
        with open(list_path, 'rb') as filehandle:
            # read the data as binary data stream
            placesList = pickle.load(filehandle)
        self.train_loss, self.val_loss, self.train_acc, self.val_acc, self.batch_size = placesList[0], placesList[1], placesList[2], placesList[3], placesList[4]

    def read_test_data(self):
        # load test data
        self.dataset_test_img = datasets.ImageFolder("dataset/test", transform = self.transform)
        self.dataloader_test_img = torch.utils.data.DataLoader(dataset=self.dataset_test_img, batch_size = 253)

    def test(self):
        # testing
        # "/content/drive/My Drive/Colab Notebooks/models/bs_100/eph_15_bs_100_1355&929_vgg16_finetune"

        image, label = (next(iter(self.dataloader_test_img)))
        print(label)
        ...
        _, pred = torch.max(y_pred.data, 1)
        print(pred)
        img = torchvision.utils.make_grid(image,nrow=12)
        ...
        print("accuracy:{:.2f}".format(self.accuracy), "recall:{:.2f}".format(self.recall), "| precision:{:.2f}".format(self.precision), "| f1_score:{:.2f}".format(self.f1_score))

# conduct command
# for modelname in ['resnet' , 'vgg16', 'googlenet']:
    # for batch_size in range(10, 110, 10):
cocoon = MODEL('vgg16', 1, 10, 'feature_extraction')
